...
class Blackjack : CardGame() {
    ...
    private fun fiveCardCharlie() {
        gameState = "five card charlie"
        updateAvailableOptions()
    }
    ...
    private fun start() {
        ...
        if (checkCards(playerCards) == 21) pass()
        else running()
    }
    ...
    private fun hit() {
        ...
        else if (playerCards.size >= 5) fiveCardCharlie()
    }
    ...
    private fun pass() {
        ...
        if (dealerCards.size == 2 && checkCards(dealerCards) == 21) {
            if (playerCards.size == 2 && checkCards(playerCards) == 21) tie() else lose()
            return
        }

        if (playerCards.size == 2 && checkCards(playerCards) == 21) {
            blackjack()
            return
        }
